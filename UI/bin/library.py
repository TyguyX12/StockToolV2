...
#TODO FIX DAY FOR ALPHA VANTAGE
...
def getSentimentData():
    ...
    sentimentDataPaths = {"Reddit": {}, "Twitter": {}}
    folderList = {
        "Reddit": "/Users/tymar/OneDrive/Documents/Capstone/Reddit/Sentiment Scores/",
        "Twitter": "/Users/tymar/OneDrive/Documents/Capstone/Twitter/Sentiment Scores/"
        }
    
    for folderName in folderList:
        for filePath in Path(folderList[folderName]).iterdir():
            date = str(dparser.parse(str(filePath),fuzzy=True).date())
            with open(filePath, newline='') as csvfile:
                spamreader = csv.reader(csvfile, delimiter=',', quotechar='"')
                for row in spamreader:
                    if row != [] and row[0] != "source":
                        stock = row[2]
                        if stock in sentimentData[folderName]:                                                     
                            sentimentData[folderName][stock][date] = row[3:]
                        else:
                            sentimentData[folderName][stock] = {date: row[3:]} 
