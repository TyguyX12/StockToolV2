...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Add_Stock`(
	p_idStockExchange int,
    p_StockSymbol varchar(45),
    p_StockName varchar(200)
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `Add_stock_exchanges`(
	p_stock_exchanges_name varchar(255)
)
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `Create_Historical_Data_Row`(
	p_Open decimal(10,2),
    p_High decimal(10,2),
    p_Low decimal(10,2),
    p_Close decimal(10,2),
    p_MarketCap decimal(15,2),
    p_StockReference int,
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `Create_Reddit_Row`(
	p_Submission_title varchar(255),
    p_Submission_name varchar(255),
    p_Upvote_ratio float,
    p_Upvotes int,
    p_Comment LONGTEXT,
    p_Comment_author LONGTEXT,
    p_Flair LONGTEXT,
    p_Created DATETIME,
    p_StockReference int  
)
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `Create_Twitter_Row`(
	IN p_username varchar(255),
    IN p_DescriptionOfUser longtext,
    IN p_Location mediumtext,
    IN p_following longtext,
    IN p_followers int,
    IN p_totaltweets int,
    IN p_RetweetCount int,
    IN p_Hashtags longtext,
    IN p_TweetText mediumtext,
    IN p_Created datetime,
    IN p_StockReference int
)
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `Historical_Data_Row`(
	p_Open decimal(10,2),
    p_High decimal(10,2),
    p_Low decimal(10,2),
    p_Close decimal(10,2),
